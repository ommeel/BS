...
class BloomFilter:
    def __init__(self, m, k, n):
        self.m = m
        self.k = k
        # 1-bit 位数组，每个字节存储 8 个 bit
        self.bit_array = bytearray((m + 7) // 8)
        # 生成 k 个随机种子，作为哈希函数的多样性来源
        self.seed = [random.randint(0, m) for _ in range(k)]

    def add_element(self, x):
        for seed in self.seed:
            hash_value = hash_func(x, seed)  # 计算哈希值
            index = hash_value % self.m  # 映射到位数组范围
            byte_index = index // 8  # 找到对应字节
            bit_index = index % 8   # 找到字节中的具体 bit
            self.bit_array[byte_index] |= (1 << bit_index)  # 将对应 bit 置 1
    ...
    def check_element(self, x):
        for seed in self.seed:
            ...
            if not (self.bit_array[byte_index] & (1 << bit_index)):
                return False  # 如果有一个 bit 为 0，则元素一定不在集合中
        return True  # 所有 bit 都为 1，则可能在集合中
